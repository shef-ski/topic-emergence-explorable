...
const initialize = (display,config) => {
	...
	origin.selectAll(".agent-topic-link")
	...
	origin.selectAll(null).data(agents).enter().append("circle")
		...
		.style("fill", function(d) { // Set fill color based on culture
...
const go = (display, config) => {
    ...
    display.selectAll("circle." + styles.node) // Select all agent circles by class
    ...
    display.selectAll("rect." + styles.node) // Select all topic rectangles
    ...
    display.selectAll(".agent-topic-link") // Select all agent-topic links
        .attr("x1", d => X(d.x)) // Update starting X: agent's x-coordinate
        .attr("y1", d => Y(d.y)) // Update starting Y: agent's y-coordinate
        .attr("x2", d => { // Update ending X: topic's x-coordinate
            return d.focussed_topic ? X(d.focussed_topic.x) : X(d.x);
        })
        .attr("y2", d => { // Update ending Y: topic's y-coordinate
            return d.focussed_topic ? Y(d.focussed_topic.y) : Y(d.y);
        });
}
