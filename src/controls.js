...
export default (controls, grid) => {
    ...
    const tg_pos = range(toggles.length).map(i => {
        const calculatedX = cfg.widgets.toggle_anchor.x + (i * cfg.widgets.toggle_horizontal_gap);
    ...
    // --- radio positions ---
    const ch_pos = range(radios.length)
        .map(i => grid.position(
    ...
    radios.forEach((radio, i) => {
        radio.position(ch_pos[i]) // Use calculated positions
