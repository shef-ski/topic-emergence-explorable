...
const initialize = () => {
	...
	// --- make topics ---
	const N_topics = param.number_of_topics.choices[param.number_of_topics.widget.value()];
	...
	topics = map(range(N_topics), i => {
		...
		return {
				...
				get size() {
					...
					return (this.initial_news_val + N_topics * 0.5 * this.network_news_val + 0.8) ** 2;
				}
			}
	});

	// --- make agents ----
	const N_agents = param.number_of_agents.choices[param.number_of_agents.widget.value()];
...
const go  = () => {
    ...
    agents.forEach(agent => {
        ...
        const evaluation = (similarity * param.weight_similarity) +
		...
		if (forgetting_probability < param.forgetting_threshold.widget.value()) {
...
const update = () => {
	each(agents,x => {x.active = x.index < param.number_of_agents.widget.value() ? true : false})
}
